...
class Weibo_oauth {
	...
	/**
	* use web proxy to access internet
	*
	* @ignore
	**/
	...
	function get($url, $parameters = array()) {
		...
		if ($this->format === 'json' && $this->decode_json) {
			//fix json_decode integer overflow bug in PHP 5.2+
			$response = preg_replace('/("[\w]+":)(\d{9,})/i', '${1}"${2}"', $response);
	...
	function http($url, $method, $postfields = NULL, $headers = array()) {
		...
		if($this->use_proxy){
        	...
        	curl_setopt($ci, CURLOPT_PROXY, $this->proxy_server.':'.$this->proxy_port);
