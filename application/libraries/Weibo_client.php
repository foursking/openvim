...
class Weibo_client
{
	...
	function __construct( $params)
	{
		...
		$this->_CI->load->library('weibo_oauth', $params);
		$this->oauth = &$this->_CI->weibo_oauth;
	}

	/**
	 * 开启代理访问
	 *
	 * 开启代理访问，SDK会将通过代理服务器发送每次请求。
	 *
	 * @access public
	 * @param bool $enable 是否开启代理访问
	 * @return void
	 */
	function set_proxy($server,$port){
			$this->oauth->use_proxy = TRUE;
        	$this->oauth->proxy_server = $server;
        	$this->oauth->proxy_port = $port;;
	}
